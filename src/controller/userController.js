...
//핸드폰로그인
export const auth_phone = async (req, res) => {
...
//휴대폰번호로 회원 조회
export const exist_phone = async (req, res) => {
  const { phoneNum } = req.body;
  try {
    const user = await User.findOne({ phoneNum });
    if (!user)
      return res.status(406).send({ result: "failure", isExisting: false });
...
//이메일로 회원 조회
export const exist_email = async (req, res) => {
